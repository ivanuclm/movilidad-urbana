...
El modelo activo para \texttt{/predict} se controla mediante la variable de entorno \texttt{LPMC\_MODEL\_VARIANT} (\texttt{xgb} por defecto). \todo{Refactorizar: el modelo a usar debería ser un parámetro de la petición POST /api/lpmc/predict (ej. \texttt{"model": "xgb"}), con XGBoost como valor por defecto. La variable de entorno es un parche de desarrollo que no debería llegar a producción.} Ninguno de los tres modelos incluye \texttt{household\_id} como variable de entrada, ya que ese identificador de hogar no está disponible en tiempo de ejecución; solo se usó durante el entrenamiento como criterio de agrupación en la validación cruzada.
