...
Responder a consultas como «qué líneas pasan por esta parada» o «cuál es el horario de una ruta para una fecha concreta» requiere cruzar varios de estos ficheros. Para obtener las líneas asociadas a una parada se encadenan tres joins: \texttt{stops.txt} $\to$ \texttt{stop\_times.txt} $\to$ \texttt{trips.txt} $\to$ \texttt{routes.txt}. Para los horarios por fecha se añade el filtro de \texttt{calendar\_dates.txt}, que indica los días en que cada servicio está activo. Mantener todos los ficheros en memoria evita releer los CSV en cada petición y elimina la necesidad de añadir una base de datos para datos que no cambian en tiempo de ejecución.
...
El feed GTFS de Toledo no incluye de forma consistente los campos opcionales \texttt{route\_color} y \texttt{route\_text\_color}, por lo que una asignación aleatoria generaría inconsistencias visuales entre sesiones: la misma línea aparecería con colores distintos en cada recarga. Para garantizar estabilidad visual, el color de cada línea se determina de forma completamente determinista mediante un hash polinómico sobre los caracteres de su identificador:

\begin{equation}
  \text{color}(\mathit{route\_id}) = \text{PALETTE}\!\left[\left|\sum_{i=0}^{n-1} \mathrm{ord}(\mathit{route\_id}[i]) \cdot 31^{i}\right| \bmod |\text{PALETTE}|\right]
\end{equation}

La función convierte el identificador de texto en un número entero en tres pasos. Primero, cada carácter se transforma en su valor ASCII mediante \texttt{ord()}, con lo que la cadena queda representada como una secuencia de enteros. Segundo, cada valor se pondera por $31^{i}$, donde $i$ es la posición del carácter en la cadena: así, cadenas con los mismos caracteres en distinto orden producen hashes diferentes (por ejemplo, \texttt{"L1"} y \texttt{"1L"} dan resultados distintos). El 31 es una base prima con buenas propiedades de distribución en hashes de cadenas, la misma que emplea \texttt{String.hashCode()} en Java. Por último, el valor absoluto evita índices negativos y el módulo sobre $|\mathrm{PALETTE}|$ garantiza que el resultado siempre caiga dentro del rango válido del vector, independientemente del valor del hash. La implementación en Python refleja fielmente la fórmula:

\begin{lstlisting}[language=Python]
PALETTE = ["#f97316", "#0ea5e9", ..., "#06b6d4", "#8b5cf6"]

def route_color(route_id: str) -> str:
    h = sum(ord(c) * (31 ** i) for i, c in enumerate(route_id))
    return PALETTE[abs(h) % len(PALETTE)]
\end{lstlisting}

La Figura~\ref{fig:gtfs_paradas} muestra la capa GTFS en la interfaz: las paradas aparecen como marcadores interactivos sobre el mapa de Toledo, y al seleccionar una se despliega un popup con su nombre, código y las líneas que la sirven coloreadas con la paleta determinista. En el panel lateral se presenta el detalle de la línea seleccionada: el trazado completo sobre el mapa, la secuencia de paradas y los horarios del día.

\begin{figure}[H]
  \centering
    % \missingfigure{Captura de la interfaz mostrando la capa GTFS: paradas como marcadores interactivos sobre el mapa de Toledo, con popups que muestran nombre, código y líneas asociadas coloreadas. Al seleccionar una línea, se muestra su trazado completo sobre el mapa, secuencia de paradas y horarios del día en el panel lateral.}
  \includegraphics[width=1.0\textwidth]{figs/GTFS_Linea5.png}
