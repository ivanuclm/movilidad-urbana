...
\subsection{Coloración de líneas}
\label{subsec:impl_gtfs_color}

El feed GTFS de Toledo incluye los campos \texttt{route\_color} y \texttt{route\_text\_color} para todas sus líneas. La implementación inicial los ignoraba y calculaba colores propios mediante un hash sobre el identificador de ruta, con el consiguiente desajuste visual respecto a la identidad cromática del operador. La solución consiste en leer ambos campos directamente del feed y aplicarlos en el panel lateral y en los chips de parada, garantizando coherencia con la señalética oficial. Cuando el feed no incluya estos campos, se genera un color de respaldo mediante un hash polinómico sobre el nombre corto proyectado al espacio HSL con saturación del 70\,\% y luminosidad del 35\,\%, valores que aseguran contraste suficiente para texto sobre cualquier tono generado.

La Figura~\ref{fig:gtfs_paradas} muestra el panel Red GTFS junto al mapa. El catálogo se organiza en un acordeón donde cada fila representa una línea física identificada por nombre corto, color y nombre largo. Al expandir una entrada, el panel muestra: el trazado de la ruta sobre el mapa en el color oficial, un diagrama vertical de paradas inspirado en los paneles de andén (terminales como puntos rellenos, intermedias como aros huecos), y una tabla de salidas agrupada por hora. Cuando una línea tiene variantes en sentidos distintos, aparece una sublista con los headsigns de cada subtrayecto. Al pulsar sobre una parada del diagrama, el mapa ejecuta un desplazamiento animado (\texttt{flyTo}) hasta esa posición.
