...
OSRM trabaja sobre un extracto de OpenStreetMap (OSM), un fichero \texttt{.osm.pbf} que contiene la geometría de calles, carreteras y caminos de un territorio junto con sus atributos de velocidad, sentido y accesibilidad por modo. Geofabrik \cite{GeofabrikDownloads} distribuye estos extractos por jerarquía geográfica, desde continentes hasta comunidades autónomas.

Se utilizó el extracto de Castilla-La Mancha, que encaja con el ámbito del proyecto tanto por el Máster (UCLM) como por el caso de estudio elegido, Toledo, ciudad para la que se encontró el feed GTFS con mayor cobertura y calidad de la región. OpenTripPlanner reutiliza el mismo extracto para la red viaria peatonal (sección~\ref{sec:impl_otp}), por lo que un único fichero sirve a dos servicios del sistema. La Tabla~\ref{tab:osm_extractos} resume los extractos evaluados durante el desarrollo.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrl}
    ...
    Comunidad de Madrid & \textasciitilde{}330 MB   & Pruebas intermedias con feed GTFS EMT Madrid \\
...
Durante la búsqueda de un feed GTFS adecuado para el transporte público, descrita en la sección~\ref{sec:impl_otp}, se trabajó temporalmente con el extracto de la Comunidad de Madrid para probar la integración con el feed de la EMT. Una vez seleccionado Toledo como caso de estudio definitivo se volvió al extracto de Castilla-La Mancha.
...
Cada perfil define las reglas de movilidad del modo correspondiente: qué tipos de vía son accesibles, a qué velocidad, con qué restricciones de giro y qué preferencias de trazado aplican. Los tres archivos de perfil estándar (\texttt{car.lua}, \texttt{bicycle.lua} y \texttt{foot.lua}) están incluidos en la imagen oficial de Docker \texttt{osrm/osrm-backend} en \texttt{/opt/}, por lo que no es necesario descargarlos ni configurarlos externamente. Todas las etapas del preprocesado se ejecutan con esa misma imagen, lo que fija la versión del software y garantiza la reproducibilidad del entorno.
...
\begin{enumerate}
  ...
  \item \textbf{Particionado} (\texttt{osrm-partition}): divide el grafo en celdas jerárquicas para el algoritmo Multi-Level Dijkstra (MLD). Esta partición es la que permite calcular rutas de larga distancia en milisegundos, al limitar la búsqueda a fronteras entre celdas en lugar de explorar el grafo completo.

  \item \textbf{Customización} (\texttt{osrm-customize}): asigna los pesos finales de cada arista sobre la estructura de celdas anterior, produciendo el grafo listo para servir peticiones de ruta.
\end{enumerate}

El resultado de las tres etapas son los ficheros \texttt{clm.osrm} y sus auxiliares, almacenados en directorios separados por perfil (\texttt{osrm-clm/car/}, \texttt{osrm-clm/bike/}, \texttt{osrm-clm/foot/}) para que los tres conjuntos de artefactos coexistan sin interferencia. Estos artefactos no se versionan en el repositorio por su tamaño; el procedimiento completo se recoge en el anexo~\ref{anx:despliegue}.
