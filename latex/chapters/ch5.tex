...
Este capítulo detalla el proceso de construcción del simulador: las decisiones técnicas concretas tomadas en cada componente, los problemas encontrados y cómo se resolvieron, y los resultados obtenidos en el entrenamiento y evaluación de los modelos de elección modal.
...
OSRM requiere un extracto de OpenStreetMap preprocesado para la región de interés. Se utilizó el de Castilla-La Mancha, distribuido por Geofabrik en formato \texttt{.osm.pbf} \cite{GeofabrikDownloads}, por tres razones concurrentes: el ámbito geográfico del caso de estudio se circunscribe a Toledo y su área urbana; el mismo extracto lo reutiliza OpenTripPlanner para la red viaria peatonal (sección~\ref{sec:impl_otp}); y el coste de preprocesado es proporcional al tamaño del grafo. La Tabla~\ref{tab:osm_extractos} compara el extracto regional con otras alternativas evaluadas durante el desarrollo.
...
La integración de OSRM evolucionó a través de tres fases sucesivas, cada una motivada por las limitaciones de la anterior.

\textbf{Fase 1: servidor de demostración oficial.} La primera implementación utilizó el servidor público de OSRM (\texttt{router.project-osrm.org}). La API acepta peticiones del tipo \texttt{/route/v1/\{profile\}/lon,lat;lon,lat} y es adecuada para verificar la conectividad y el formato de respuesta. El problema es que el servidor de demostración está capado: aunque la URL incluye el perfil (\texttt{car}, \texttt{bike}, \texttt{foot}), devuelve en la práctica el mismo resultado para los tres, correspondiente al enrutado por carretera. Dado que el objetivo central del simulador es la comparación multimodal con atributos diferenciados, esta limitación hacía inviable el servidor de demostración para el caso de uso real.

\textbf{Fase 2: instancias públicas FOSSGIS.} La alternativa encontrada fueron las instancias públicas mantenidas por FOSSGIS, que sí ofrecen perfiles diferenciados mediante rutas distintas (\texttt{routing.openstreetmap.de/routed-car}, \texttt{/routed-bike}, \texttt{/routed-foot}). Esta fase permitió verificar la viabilidad del enfoque multimodal antes de comprometerse con un despliegue local, y sirvió para definir el contrato de la API que el backend debía consumir. Los servicios públicos presentan, sin embargo, latencia variable, dependencia de disponibilidad externa y no permiten fijar la versión de los datos, lo que dificulta la reproducibilidad de los experimentos.

\textbf{Fase 3: instancia local con Docker.} La solución definitiva fue desplegar OSRM localmente mediante la imagen oficial \texttt{osrm/osrm-backend}, con un contenedor independiente por perfil. Esta decisión extiende al ámbito de los perfiles la misma lógica que motivó el uso de OSRM frente a APIs comerciales: reproducibilidad, control de versión y ausencia de límites de cuota. La imagen oficial incluye los tres perfiles estándar en \texttt{/opt/}: \texttt{car.lua}, \texttt{bicycle.lua} y \texttt{foot.lua}, lo que elimina la necesidad de obtenerlos o configurarlos externamente. Los tres contenedores se integran en el entorno Docker Compose descrito en la sección~\ref{sec:arch_infra}.

\subsection{Pipeline de preprocesado por perfil}
\label{subsec:impl_osrm_pipeline}

El preprocesado OSRM consta de tres etapas que deben ejecutarse una sola vez por perfil, o cada vez que se actualice el extracto OSM. Todas las etapas se ejecutan con la imagen oficial de Docker, lo que fija la versión del software y garantiza la reproducibilidad del entorno.
...
\begin{enumerate}
  ...
  \item \textbf{Customización} (\texttt{osrm-customize}): ajusta los pesos de las aristas sobre la estructura de celdas generada en la etapa anterior, produciendo el grafo listo para servir peticiones de ruta.
\end{enumerate}
...
La integración del transporte público requirió localizar un feed GTFS adecuado para el caso de estudio. La fuente empleada es el Punto de Acceso Nacional de Transporte Multimodal (NAP), gestionado por el Ministerio de Transportes y Movilidad Sostenible, que centraliza datos GTFS de operadores de transporte de toda España \cite{NAPHome}. El NAP fue presentado públicamente en junio de 2021 y se ha consolidado como la referencia principal para la obtención de datos de transporte público estructurados en el país.
...
Un detalle relevante durante el proceso: el feed inicialmente descargado correspondía al operador «Autobuses Toledo». Al descargar una versión posterior, el operador había cambiado de denominación a UNAUTO S.L. (Grupo Ruiz). El formato de los ficheros GTFS permaneció idéntico (\texttt{agency.txt}, \texttt{calendar\_dates.txt}, \texttt{routes.txt}, \texttt{shapes.txt}, \texttt{stop\_times.txt}, \texttt{stops.txt}, \texttt{trips.txt}), por lo que el cambio no afectó al pipeline de carga ni al planificador.
